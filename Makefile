...
build-alpine:
	docker build --platform linux/amd64 --rm --force-rm -t gfnord/my-bloody-jenkins $(DEFAULT_BUILD_ARGS) --build-arg=FROM_TAG=$(LTS_VERSION)-alpine .

build-debian:
	docker build --platform linux/amd64 --rm --force-rm -t gfnord/my-bloody-jenkins $(DEFAULT_BUILD_ARGS) --build-arg=FROM_TAG=$(LTS_VERSION) .

build-jdk21:
	docker build --platform linux/amd64 --rm --force-rm -t gfnord/my-bloody-jenkins $(DEFAULT_BUILD_ARGS) --build-arg=FROM_TAG=$(LTS_VERSION)-jdk21 .
