...
docker-build(){
    local from_tag=${1}
    local to_tag=${2}
    echo "docker build --rm --force-rm -t gfnord/my-bloody-jenkins:${to_tag} --build-arg=FROM_TAG=${from_tag} ."
    docker build --rm --pull --force-rm -t gfnord/my-bloody-jenkins:${to_tag} --build-arg=FROM_TAG=${from_tag} .
    docker tag gfnord/my-bloody-jenkins:${to_tag} ghcr.io/gfnord/my-bloody-jenkins:${to_tag}
    echo "docker push gfnord/my-bloody-jenkins:${to_tag}"
    docker push gfnord/my-bloody-jenkins:${to_tag}
    docker push ghcr.io/gfnord/my-bloody-jenkins:${to_tag}
}
